...
class AttentionCharAccuracy(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, y_true_length, sample_weight=None):
        super().update_state(attention_character_accuracy(y_true, y_pred, y_true_length), sample_weight)


def attention_character_accuracy(y_true, y_pred, y_true_length):
    ...
    def calc(y_true, y_pred, y_true_length):
        ...
        def loop_body(i, label, predict, length, result):
            ...
            matching_chars = tf.reduce_sum(matching_chars) / (word_length)
