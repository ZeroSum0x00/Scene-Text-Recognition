...
class EntropyWordAccuracy(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, y_true_length, sample_weight=None):
        super().update_state(entropy_accuracy(y_true, y_pred, y_true_length), sample_weight)


def entropy_accuracy(y_true, y_pred, y_true_length):
    def calc(y_true, y_pred, y_true_length):
        ...
        def loop_body(i, label, predict, length, result):
            ...
            matching_word  = tf.cast(tf.equal(matching_chars, word_length + 1), dtype=tf.int32)
